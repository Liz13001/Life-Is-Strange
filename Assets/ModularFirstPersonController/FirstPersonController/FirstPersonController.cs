...
// "Added PS4 controller support (right stick camera, JoystickButton1 jump, JoystickButton10 sprint)" || version 1.0.2
// "Added Xbox controller support, auto-detects controller type" || version 1.0.3
// "Added controller zoom (Xbox: B, PS4: Kreis)" || version 1.0.4
// "Jump() und CheckGround() nutzen jetzt transform.up statt Welt-Y-Achse,
//  damit die Kontrolle bei gekippter Gravity-Orientation (GravityWalker.cs)
//  korrekt funktioniert. Verhalten in Szenen ohne Kippung unverändert." || version 1.0.5 (custom)
...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (cameraCanMove)
        {
            float lookX = Input.GetAxis("Mouse X") + Input.GetAxis("ControllerX");
            float lookY = Input.GetAxis("Mouse Y") - Input.GetAxis("ControllerY");
        ...
        if (enableZoom)
        {
            if ((Input.GetKeyDown(zoomKey) || GetControllerZoom()) && !holdToZoom && !isSprinting)
            {
                if (!isZoomed)
                    isZoomed = true;
                else
            ...
            {
                if (Input.GetKeyDown(zoomKey) || GetControllerZoom())
                    isZoomed = true;
                else if (Input.GetKeyUp(zoomKey) || GetControllerZoomUp())
                    isZoomed = false;
            }
        ...
        if (enableJump && (Input.GetKeyDown(jumpKey) || GetControllerJump()) && isGrounded)
        {
            Jump();
        }
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            bool sprintInput = Input.GetKey(sprintKey) || GetControllerSprint();

            // GEÄNDERT: velocityChange wird jetzt im LOKALEN Koordinatensystem
            // berechnet (relativ zu transform.up/right/forward) statt in Welt-Koordinaten.
            // Grund: bei gekippter Gravity-Orientation (Wand/Decke) ist die Welt-Y-Achse
            // nicht mehr die "echte" Oben/Unten-Achse - das alte "velocityChange.y = 0"
            // hat dadurch die eigentliche Gravitationsachse mit überschrieben und die
            // Custom-Gravity aus GravityWalker.cs sabotiert. Bei unrotiertem Objekt
            // (transform.up == Vector3.up) identisch zum ursprünglichen Verhalten.

            if (enableSprint && sprintInput && sprintRemaining > 0f && !isSprintCooldown)
            {
                Vector3 targetLocalVelocity = targetVelocity * sprintSpeed;
                Vector3 currentLocalVelocity = transform.InverseTransformDirection(rb.linearVelocity);
                Vector3 localVelocityChange = targetLocalVelocity - currentLocalVelocity;
                ...
                localVelocityChange.y = 0;

                if (localVelocityChange.x != 0 || localVelocityChange.z != 0)
                {
                    isSprinting = true;
                ...
                rb.AddForce(transform.TransformDirection(localVelocityChange), ForceMode.VelocityChange);
            }
            else
            {
                isSprinting = false;
                ...
                Vector3 targetLocalVelocity = targetVelocity * walkSpeed;
                Vector3 currentLocalVelocity = transform.InverseTransformDirection(rb.linearVelocity);
                Vector3 localVelocityChange = targetLocalVelocity - currentLocalVelocity;
                localVelocityChange.x = Mathf.Clamp(localVelocityChange.x, -maxVelocityChange, maxVelocityChange);
                localVelocityChange.z = Mathf.Clamp(localVelocityChange.z, -maxVelocityChange, maxVelocityChange);
    ...
    // Xbox: A / PS4: Kreuz
    private bool GetControllerJump()
    {
    ...
    // Xbox: L3 / PS4: L3
    private bool GetControllerSprint()
    {
        string[] joysticks = Input.GetJoystickNames();
        foreach (string j in joysticks)
        {
    ...
    // Xbox: B / PS4: Kreis
    private bool GetControllerZoom()
    {
        string[] joysticks = Input.GetJoystickNames();
        foreach (string j in joysticks)
        {
    ...
    // Xbox: B loslassen / PS4: Kreis loslassen
    private bool GetControllerZoomUp()
    {
        string[] joysticks = Input.GetJoystickNames();
        foreach (string j in joysticks)
        {
    ...
    {
        // GEÄNDERT: nutzt jetzt transform.up statt Welt-Y-Subtraktion,
        // damit der Ground-Check bei gekippter Gravity-Orientation korrekt bleibt.
        // Bei unrotiertem Objekt identisch zum ursprünglichen Verhalten.
        Vector3 origin = transform.position - transform.up * (transform.localScale.y * .5f);
        Vector3 direction = transform.TransformDirection(Vector3.down);
        float distance = (transform.localScale.y * .5f) + 0.2f;
    ...
    private void Jump()
    {
        // GEÄNDERT: springt jetzt entlang transform.up statt fester Welt-Y-Achse,
        // damit der Sprung bei gekippter Gravity-Orientation (Wand/Decke) in die
        // richtige lokale "Oben"-Richtung geht. Bei unrotiertem Objekt identisch
        // zum ursprünglichen Verhalten (transform.up == Vector3.up).
        if (isGrounded)
        {
...
public class FirstPersonControllerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("version 1.0.4", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleCenter, fontStyle = FontStyle.Normal, fontSize = 12 });
        EditorGUILayout.Space();
